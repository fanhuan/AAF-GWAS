...
#Check for NaNs
c(min(output_gen$scoreLS),min(output_gen$scoreGLS))
c(max(output_gen$scoreLS),max(output_gen$scoreGLS))
#If no NA
...
write.table(rankGLS_pattern, paste("top10_top-10_GLS_k",k,".pattern", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE)
