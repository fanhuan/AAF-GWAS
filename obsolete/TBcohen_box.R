...
mutation_kmers <- read.csv("rpoB_mutations_k17.kmer", header = F, col.names =c('kmer','freq'),sep="", stringsAsFactors = F)
mutation_kmerPatterns <- read.csv("rpoB_mutations_k17.pattern", header = F, col.names =c('kmer','pattern'),sep="", colClasses=c(rep("factor",2)))
...
Y <- read.fwf(file = "TBcohen_k17_n1_p1_kmerPattern.stats", widths = array(1, c(1, p)), header = F)
...
#Save the scores
kmerPattern.stats <- read.table(file = "TBcohen_k17_n1_p1_kmerPattern.stats", colClasses = c("character", "integer"), col.names = c("pattern", "freq"))
...
write.csv(total_score, paste("TBcohen_k17_n1_p1_scores_gen.csv", sep = ""), row.names = FALSE)
...
write.csv(total_score, paste("TBcohen_k17_n1_p1_scores_cul.csv", sep = ""), row.names = FALSE)
